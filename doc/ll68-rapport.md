...
Dit document beschrijft onze bevindingen bij het koppelen van de Portbase webhook event API aan de VGU-demo. We benoemen hier de pijnpunten, zodat deze als input kunnen dienen voor verbeteringen aan de architectuur, componenten en documentatie. 
...
De al bestaande VGU-demo applicatie illustreert het gebruik van iSHARE-authenticatie / -autorisatie om Veilige Goederen Uitgifte te  demonstreren en het gebruik van Apache Pulsar om gebeurtenissen in het transport te communiceren.

In LL68 is een koppeling gemaakt met het Portbase-testsysteem om deze demo uit te breiden met container transport via een schip.  Met behulp van de door Portbase geleverde webhook- en test-API zijn de volgende events toegevoegd: de Gate-In bij de haven terminal, het laden van de container op het schip, en het vertrek van dat schip.
