...
(def opts-spec
  {"STORE" ["Database file" :str :default "/tmp/dil-demo.edn" :in [:store :file]]
   ...
   ;; for event endpoints
   "BASE_URL" ["Base URL" :str :default "http://localhost:8080" :in [:base-url]]

   ;; basic auth for multiple demo environments
   ...
   "PMS_NAME"              ["PMS Name" :str :default "Port of Rotterdam" :in [:pms :site-name]]
   ...
   "PORTBASE_API_BASE_URL" ["Portbase events base URL" :str :in [:portbase :api-base-url]]
   "PORTBASE_API_KEY"      ["Portbase events API key" :str :in [:portbase :api-key]]

   "MAX_ORDERS" ["Maximum number of orders to retain per demo" :int :default 100]})
